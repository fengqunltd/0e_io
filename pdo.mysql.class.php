...
class pdo_mysql{
     ...
     *      普通模式： 'tb_member, tb_money'
     *      数组模式： array('tb_member', 'tb_money')
     * string/array $fields 需要查询的数据库字段，允许为空，默认为查找全部，两种传值模式
     *      普通模式： 'username, password'
     *      数组模式： array('username', 'password')
     * string/array $sqlwhere 查询条件，允许为空，两种传值模式
     *      普通模式： 'and type = 1 and username like "%os%"'
     *      数组模式： array('type = 1', 'username like "%os%"')
     ...
     *      普通模式： 'username = "test", type = 1, dt = now()'
     *      数组模式： array('username = "test"', 'type = 1', 'dt = now()')
     */
